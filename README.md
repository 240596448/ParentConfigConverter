...
- Заголовок:  
`{6,0,3,ГуидПоддержки,0,ГУИД,"2.5.12.147","Фирма ""1С""","УправлениеПредприятием",137634,...`
  ...
  - третья цифра `3` - количество поставок в файле. При нескольких поставках принимает значения 2, 3 и т.д.
  - `ГуидПоддержки` - идентификатор поддержки. Используется в БД таблице `Config` в поле `FileName`. Об [особенностях](#особенности-хранения-поддержки-в-бд) написано ниже.
  - четвертая цифра (вероятно) всегда равна второй.
  - ГУИД - неизвестно.
  - `"2.5.12.147"` - версия поставки.
  - `"Фирма ""1С"""` - поставщик.
  - `"УправлениеПредприятием"` -  имя поддерживаемой конфигурации; должно совпадать с именем файла поставки в каталоге `Ext/ParentConfigurations/*.cf`
  - `137634` - количество поддерживаемых объектов в каждом разделе поддержки.
...
### Особенности хранения поддержки в БД
