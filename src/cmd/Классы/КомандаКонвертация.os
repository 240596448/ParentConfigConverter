...
Процедура ОписаниеКоманды(Команда) Экспорт
	...
	Команда.Опция("dump-cache", "", "Сохранять пары Name-Id, полученные из дампа, в отдельный файл (например, src/ConfigID.txt)")
				.ТСтрока();
...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	ИмяФайлаКэшаИдентификаторов = Команда.ЗначениеОпции("dump-cache");
	...
	Если ЗначениеЗаполнено(ИмяФайлаКэшаИдентификаторов) Тогда
		Метаданные.СоздатьКэш(ИмяФайлаКэшаИдентификаторов);
	КонецЕсли;

	Конвертор = Новый Конвертор(КаталогРепозитория);
	Ошибка = Конвертор.ВыполнитьКонвертацию(Метаданные, ИмяВременногоФайлаИсходный, ИмяВременногоФайлаЦелевой);
